    ...
    switch (action.type) {
        ...
        case DRAG_END:
            if (action.destinationDroppable === "champSelect") {
                ...
                if (action.destinationDroppable in getSelections(rootState) && action.sourceDroppable === "champSelect") {
                    const newState = [...state.filter((champ) => champ !== action.sourceDraggable), getSelection(rootState, action.destinationDroppable)];
...
const selections = (state = {}, action) => {
    switch (action.type) {
        case DRAG_END:
            if (action.destinationDroppable === "champSelect") {
                const key = Object.keys(state).find(key => state[key] === action.sourceDraggable)
                const {[key]: removed, ...rest} = state
                return rest
            } else {
                if (action.destinationDroppable in state && action.sourceDroppable !== "champSelect") {
                    return {
                        ...state,
